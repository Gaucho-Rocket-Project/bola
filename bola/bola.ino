...
// Motor Constants
unsigned long lastMotorTime = 0, motorInterval = 1000; // ms between motor triggers

// --- Helpers ---
uint32_t usToDuty(int us){return (uint32_t)us * ((1UL << escRes) - 1) / 20000;}

static float lpf(float prev_lpf_val, float current_measurement, float beta){return beta * current_measurement + (1.0f - beta) * prev_lpf_val;}
...
int getTheta4(int theta2)
{
  int offset = 70;
  int index;
  if (theta2 > 110){index = 110 - offset;}
  else if (theta2 < 70){index = 70 - offset;}
  else{index = theta2 - offset;}
...
void triggerFirstMotor()
{
  digitalWrite(primaryMotorPin, HIGH);
  Serial.println("Triggered first motor");
  delay(100);
  digitalWrite(primaryMotorPin, LOW);
}

void triggerSecondMotor()
{
  // digitalWrite(primaryMotorPin, LOW);
  digitalWrite(secondaryMotorPin, HIGH);
  Serial.println("Triggered second motor");
  delay(100);
  digitalWrite(secondaryMotorPin, LOW);
}

void triggerLegs()
{
  ledcWrite(leg_pin, usToDuty(1500));
  Serial.println("Deployed landing legs");
}
